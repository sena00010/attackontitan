  ...
  const postList = postSnapshot.docs.map(doc => doc.data());
    ...
    return (
        <div className={styles.main}>
                ...
                {data.map((item: any) => (
                    ...
                    <div key={item.id} className={styles.postContainer}>
                        <div className={styles.header}>
                            <Image src={item.post.postImage[0]} style={{borderRadius: "100%"}} width={50} height={50}
                                   alt={item.post.postImage[0]}/>
                            <div>userName</div>
                        </div>
                        <div> {item.post.postContent}</div>
                        <div>
                            <Image src={item.post.postImage[1]}  width={300} height={300}
            ...
            <PostCreated opened={openPost} setClosed={setOpenPost}/>
        </div>
    );
